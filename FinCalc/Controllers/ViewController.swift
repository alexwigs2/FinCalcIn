...
//  FinCalc
//
//  Created by Alex Wigsmoen on 8/29/21.
//
...
class ViewController: UIViewController, UITextFieldDelegate {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if segue.identifier == "goToResult" {
            ...
            destinationVC.summary = "Future value starting at $\(pvalue.text ?? "") with \(inter.text ?? "")% interest for \(year.text ?? "") years with $\(peri.text ?? "") added every period."
